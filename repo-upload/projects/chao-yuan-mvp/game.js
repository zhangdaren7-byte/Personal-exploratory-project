...
const art = {
  shore: loadArt("assets/pixel-shore-map.jpg"),
  abyss: loadArt("assets/pixel-abyss-map.jpg"),
  hero: loadArt("assets/pixel-hero-sheet.png"),
  elements: loadArt("assets/pixel-elements-sheet.png"),
  interactables: loadArt("assets/pixel-interactables-sheet.png"),
};

const SPRITE_CELL = { w: 384, h: 512 };
const ELEMENT_CELL = { w: 384, h: 512 };
const spriteFrames = {
...
function drawElement(key, x, y, w, h) {
  ...
  const sx = sprite.col * ELEMENT_CELL.w + sprite.sx;
  const sy = sprite.row * ELEMENT_CELL.h + sprite.sy;
  ctx.save();
  ctx.imageSmoothingEnabled = false;
  ctx.drawImage(art.elements, sx, sy, sprite.sw, sprite.sh, x - w / 2, y - h / 2, w, h);
  ctx.restore();
}
...
function drawSpritePlayer(water) {
  ...
  const sx = f.col * SPRITE_CELL.w + f.sx;
  const sy = f.row * SPRITE_CELL.h + f.sy;
  ...
  ctx.scale(scaleX, 1);
  ctx.drawImage(art.hero, sx, sy, f.sw, f.sh, -w / 2, -h + 33, w, h);
  ctx.restore();
...
function drawMiniHeroFrame(frameKey, x, y, w, h) {
  ...
  ctx.save();
  ctx.imageSmoothingEnabled = false;
  ctx.drawImage(art.hero, sx, sy, f.sw, f.sh, x - w / 2, y - h + 14, w, h);
  ctx.restore();
}
